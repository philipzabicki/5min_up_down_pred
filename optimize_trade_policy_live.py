...
from common_config_utils import load_json_object, path_to_portable_str
from polymarket_fee_utils import (
    polymarket_taker_fee_fraction_of_notional,
    polymarket_taker_fee_usdc_from_notional,
)
from project_config import load_live_profile, load_runtime_artifact_paths
...
def replay_policy(backtest_frame, runtime_config, *, order_price_cap):
    if str(runtime_config.get("mode", "ev")).strip().lower() != "ev":
        raise ValueError("optimize_trade_policy_live.py currently optimizes EV mode only.")
