...
ROW_FILTER_COMPARE_DEFAULT_OUTPUT_DIR = active_asset_path(
    "data/analysis/lgbm_row_filter_compare/{asset}"
)
...
SAVE_BEST_OOF = False
RUN_ROW_FILTER_COMPARE_BEFORE_WEIGHT_SEARCH = True
ROW_FILTER_COMPARE_OUTPUT_DIR = ROW_FILTER_COMPARE_DEFAULT_OUTPUT_DIR
ROW_FILTER_COMPARE_PARAMS_SOURCE = "optuna"
ROW_FILTER_COMPARE_CV_FOLDS = CV_FOLDS
ROW_FILTER_COMPARE_TEST_TO_TRAIN_RATIO = TEST_TO_TRAIN_RATIO
ROW_FILTER_COMPARE_N_ESTIMATORS = N_ESTIMATORS
ROW_FILTER_COMPARE_EARLY_STOPPING_ROUNDS = EARLY_STOPPING_ROUNDS
ROW_FILTER_COMPARE_DEVICE_TYPE = DEVICE_TYPE
ROW_FILTER_COMPARE_N_JOBS = LGBM_N_JOBS
ROW_FILTER_COMPARE_PREVIEW_ROWS = DEFAULT_PREVIEW_ROWS
ROW_FILTER_COMPARE_PRIMARY_SCOPE = "decision_rows"
ROW_FILTER_COMPARE_PRIMARY_METRIC = "balanced_accuracy"
...
def validate_config():
    ...
    if ROW_FILTER_COMPARE_PARAMS_SOURCE not in {"optuna", "default"}:
        raise ValueError(
            "ROW_FILTER_COMPARE_PARAMS_SOURCE must be one of: 'optuna', 'default'."
        )
    if ROW_FILTER_COMPARE_CV_FOLDS < 2:
        raise ValueError("ROW_FILTER_COMPARE_CV_FOLDS must be >= 2.")
    if ROW_FILTER_COMPARE_N_ESTIMATORS < 1:
        raise ValueError("ROW_FILTER_COMPARE_N_ESTIMATORS must be >= 1.")
    if ROW_FILTER_COMPARE_EARLY_STOPPING_ROUNDS < 0:
        raise ValueError("ROW_FILTER_COMPARE_EARLY_STOPPING_ROUNDS must be >= 0.")
    if ROW_FILTER_COMPARE_PREVIEW_ROWS < 1:
        raise ValueError("ROW_FILTER_COMPARE_PREVIEW_ROWS must be >= 1.")
    if not (0.0 < float(ROW_FILTER_COMPARE_TEST_TO_TRAIN_RATIO) < 1.0):
        raise ValueError("ROW_FILTER_COMPARE_TEST_TO_TRAIN_RATIO must be in (0, 1).")
    if ROW_FILTER_COMPARE_DEVICE_TYPE not in {"gpu", "cpu"}:
        raise ValueError("ROW_FILTER_COMPARE_DEVICE_TYPE must be one of: 'gpu', 'cpu'.")
...
def row_filter_eval_scope(variant_name):
    if variant_name == ROW_MODE_ALL_ROWS:
        return EVAL_SCOPE_ALL_ROWS
    if variant_name == ROW_MODE_DECISION_ONLY:
        return EVAL_SCOPE_DECISION_ONLY
    raise ValueError(f"Unsupported row filter variant: {variant_name}")
...
def metric_summary_from_row_filter_fold_df(fold_df, scope_name):
    summary = {}
...
def build_row_filter_variant_summary_row(variant_name, variant_summary):
    row = {
...
def compare_row_filter_scope_metrics(base_variant, candidate_variant, scope_name):
    base_metrics = base_variant["oof_metrics"][scope_name]["metrics"] or {}
    candidate_metrics = candidate_variant["oof_metrics"][scope_name]["metrics"] or {}
...
def resolve_row_filter_recommendation(comparison_payload):
    primary_comparison = comparison_payload[ROW_FILTER_COMPARE_PRIMARY_SCOPE]
    primary_delta = primary_comparison["delta_candidate_minus_base"].get(
        ROW_FILTER_COMPARE_PRIMARY_METRIC
    )
    if primary_delta is None:
        return None
    if abs(primary_delta) <= 1e-15:
        return "tie"
    if ROW_FILTER_COMPARE_PRIMARY_METRIC in LOWER_IS_BETTER_METRICS:
        return ROW_MODE_DECISION_ONLY if primary_delta < 0.0 else ROW_MODE_ALL_ROWS
    return ROW_MODE_DECISION_ONLY if primary_delta > 0.0 else ROW_MODE_ALL_ROWS
...
def evaluate_row_filter_variant(
    ...
    for fold in folds:
        ...
        selected = select_strategy_fold_indices(
            ...
            eval_scope=row_filter_eval_scope(variant_name),
        ...
        model = build_lgbm_model(
            n_estimators=int(ROW_FILTER_COMPARE_N_ESTIMATORS),
        ...
        if int(ROW_FILTER_COMPARE_EARLY_STOPPING_ROUNDS) > 0:
            fit_kwargs["callbacks"] = [
                lgb.early_stopping(
                    stopping_rounds=int(ROW_FILTER_COMPARE_EARLY_STOPPING_ROUNDS),
        ...
        best_iteration = int(model.best_iteration_ or int(ROW_FILTER_COMPARE_N_ESTIMATORS))
        ...
        scoped_metrics = evaluate_weighted_prediction_scopes(
        ...
        print(
            f"row-filter variant={variant_name} fold={fold['fold_id']} "
    ...
    oof_scope_metrics = evaluate_weighted_prediction_scopes(
    ...
    return {
        ...
        "cv_scope_metrics": {
            scope_name: metric_summary_from_row_filter_fold_df(fold_df, scope_name)
...
def run_row_filter_compare_preflight():
    if not RUN_ROW_FILTER_COMPARE_BEFORE_WEIGHT_SEARCH:
        return None

    output_dir = Path(ROW_FILTER_COMPARE_OUTPUT_DIR) / make_run_timestamp()
    ...
    folds = make_walk_forward_folds(
        n_rows=len(x),
        n_folds=int(ROW_FILTER_COMPARE_CV_FOLDS),
        test_to_train_ratio=float(ROW_FILTER_COMPARE_TEST_TO_TRAIN_RATIO),
    )
    param_overrides = build_model_param_overrides(
        param_source=ROW_FILTER_COMPARE_PARAMS_SOURCE,
        device_type=ROW_FILTER_COMPARE_DEVICE_TYPE,
        n_jobs=ROW_FILTER_COMPARE_N_JOBS,
    )
    monotone_constraint_summary = summarize_lgbm_monotone_constraints(x.columns)

    print(
        f"row-filter compare preflight | dataset={data_path} rows={len(df)} "
        f"features={x.shape[1]} decision_rows={decision_summary['rows']} "
        f"decision_share={decision_summary['share']:.4f}"
    )
    print(
        "row-filter compare config | "
        f"params_source={ROW_FILTER_COMPARE_PARAMS_SOURCE} "
        f"device_type={ROW_FILTER_COMPARE_DEVICE_TYPE} "
        f"cv_folds={ROW_FILTER_COMPARE_CV_FOLDS} "
        f"test_to_train_ratio={float(ROW_FILTER_COMPARE_TEST_TO_TRAIN_RATIO):.4f} "
        f"float_precision={modeling_float_dtype_name}"
    )
    print(
        "row-filter compare monotone_constraints | "
        f"{format_lgbm_monotone_constraint_summary(monotone_constraint_summary)}"
    )

    variant_results = {}
    fold_metrics_frames = []
    for variant_name in (ROW_MODE_ALL_ROWS, ROW_MODE_DECISION_ONLY):
        variant_results[variant_name] = evaluate_row_filter_variant(
            variant_name=variant_name,
            x=x,
            y=y,
    ...
    variant_summary_rows = pd.DataFrame(
        [
            build_row_filter_variant_summary_row(variant_name, result)
            for variant_name, result in variant_results.items()
        ]
    )
    fold_metrics_df = pd.concat(fold_metrics_frames, ignore_index=True)
    comparison_payload = {
        scope_name: compare_row_filter_scope_metrics(
    ...
    recommended_variant = resolve_row_filter_recommendation(comparison_payload)
    ...
    summary_payload = {
        ...
        "walk_forward": {
            ...
            "test_to_train_ratio": float(ROW_FILTER_COMPARE_TEST_TO_TRAIN_RATIO),
            "n_estimators": int(ROW_FILTER_COMPARE_N_ESTIMATORS),
            "early_stopping_rounds": int(ROW_FILTER_COMPARE_EARLY_STOPPING_ROUNDS),
            "params_source": str(ROW_FILTER_COMPARE_PARAMS_SOURCE),
            "device_type": str(ROW_FILTER_COMPARE_DEVICE_TYPE),
        ...
        "comparison": {
            "primary_scope": ROW_FILTER_COMPARE_PRIMARY_SCOPE,
            "primary_metric": ROW_FILTER_COMPARE_PRIMARY_METRIC,
        ...
        "artifacts": {
            ...
            "oof_predictions_head_csv": path_to_portable_str(
                output_dir / f"oof_predictions_head{int(ROW_FILTER_COMPARE_PREVIEW_ROWS)}.csv"
            ),
            "oof_predictions_tail_csv": path_to_portable_str(
                output_dir / f"oof_predictions_tail{int(ROW_FILTER_COMPARE_PREVIEW_ROWS)}.csv"
            ),
        },
    }
    ...
    oof_export.head(int(ROW_FILTER_COMPARE_PREVIEW_ROWS)).to_csv(
        output_dir / f"oof_predictions_head{int(ROW_FILTER_COMPARE_PREVIEW_ROWS)}.csv",
        index=False,
    )
    oof_export.tail(int(ROW_FILTER_COMPARE_PREVIEW_ROWS)).to_csv(
        output_dir / f"oof_predictions_tail{int(ROW_FILTER_COMPARE_PREVIEW_ROWS)}.csv",
        index=False,
    )

    for variant_name in (ROW_MODE_ALL_ROWS, ROW_MODE_DECISION_ONLY):
        decision_scope = variant_results[variant_name]["oof_metrics"][
            ROW_FILTER_COMPARE_PRIMARY_SCOPE
        ]
        all_rows_scope = variant_results[variant_name]["oof_metrics"]["all_rows"]
        print(
            f"row-filter summary variant={variant_name} "
    ...
    print(
        "row-filter compare preflight | "
        f"winner={recommended_variant} scope={ROW_FILTER_COMPARE_PRIMARY_SCOPE} "
        f"metric={ROW_FILTER_COMPARE_PRIMARY_METRIC} "
        f"output_dir={path_to_portable_str(output_dir)}"
    )
    return summary_payload


def build_model_param_overrides(param_source=None, *, device_type=None, n_jobs=None):
    ...
    params["device_type"] = str(DEVICE_TYPE if device_type is None else device_type)
    resolved_n_jobs = LGBM_N_JOBS if n_jobs is None else n_jobs
    if resolved_n_jobs is not None:
        params["n_jobs"] = int(resolved_n_jobs)
    return params
...
def main():
    validate_config()
    row_filter_compare_summary = run_row_filter_compare_preflight()
    ...
    if row_filter_compare_summary is not None:
        payload["row_filter_compare_preflight"] = row_filter_compare_summary
    payload["artifacts"] = {
